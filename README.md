...
This folder is a **personal working copy** of monorepo `e2e-testing/`. Edit and run app tests here; treat the monorepo as read-only reference for UI source and selectors.

| | Path |
| --- | --- |
| This workspace | `/Users/alexcopperman/Downloads/Opentrons_General/opentrons-app-e2e` |
| Monorepo (reference) | `/Users/alexcopperman/Downloads/Opentrons_General/opentrons` |

**Primary commands (desktop app):**
...
Requires an installed Opentrons app and a connected Flex (USB or Wi-Fi). For `fake-robot` / `make -C app dev`, set in `.env` (see `.env.example`):

```bash
OPENTRONS_ROOT=/Users/alexcopperman/Downloads/Opentrons_General/opentrons
```

Agents: use the project skill `.cursor/skills/app-e2e/` when working on app tests. Look up UI in `app/src/`, `components/src/`, and `app-shell/` under the monorepo path above.

Upstream changes manually into monorepo `e2e-testing/` when ready — there is no automatic sync.
...
2. **Run app tests in headed mode (this workspace's main flow):**

   ```bash
   make configure-robot
   make test-app-headed
   ```

3. **Or run PD tests to recreate a failure locally:**

   ```bash
   make test-pd-local
   ```

4. **Troubleshoot in headed mode and only running the failing test:**

   ```bash
   make troubleshoot
   ```

5. If you know the test name you want to run, this is how you watch it run in headed mode:

   ```bash
   make test-app-headed PYTEST_ARGS="-k test_name"
   # or for PD:
...
Use `--robot-profile fake-robot`, `--robot-name QA1Potato`, `--robot-ip 10.x.x.x`, or positional args via `main_script.py`.

### Flex ODD (touchscreen over CDP)

Enable **Developer Tools** on the robot (Robot Settings), then attach Playwright to port **9223**:

```bash
make configure-robot                             # set ROBOT_IP in .env
make test-odd-headed                             # uses ROBOT_IP from .env
make test-odd-headed ROBOT_IP=10.14.19.200        # any Flex IP for this run
