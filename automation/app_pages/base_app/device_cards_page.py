...
class DeviceCardsPage(AppBasePage):
    ...
    def read_about_module(self, serial_prefix: str, module_model: str) -> tuple[str, str | None]:
    ...
    def _module_card(self, serial_prefix: str) -> Locator:
        ...
        return self.page.get_by_test_id(re.compile(rf"^ModuleCard_{re.escape(serial_prefix)}"))

    def _module_overflow_button(self, serial_prefix: str) -> Locator:
        """Overflow control lives on ``ModuleCard_overflow_btn_<serial>``."""
        return (
            self._module_card(serial_prefix)
            .first.get_by_test_id(re.compile(rf"^ModuleCard_overflow_btn_{re.escape(serial_prefix)}"))
    ...
    def _instrument_card(self, label: str) -> Locator:
        ...
        label_pattern = re.compile(rf"^({'|'.join(re.escape(candidate) for candidate in labels)})$")
    ...
    def _instrument_overflow_button(self, label: str) -> Locator:
        ...
        return self._instrument_card(label).get_by_role("button", name=re.compile("InstrumentCard_overflowMenu")).first
    ...
    def _open_module_overflow(self, serial_prefix: str) -> bool:
        ...
        if overflow.is_disabled():
            print(f"  Skipping module with prefix '{serial_prefix}' — overflow menu disabled.")
            return False
        overflow.click()
    ...
    def _click_module_menu_button(self, serial_prefix: str, *names: str) -> bool:
    ...
    def _open_module_temp_slideout(
        ...
        *menu_names: str,
        ...
        set_names = tuple(name for name in menu_names if not name.lower().startswith("deactivate"))
        deactivate_names = tuple(name for name in menu_names if name.lower().startswith("deactivate"))
        if not set_names:
            return False
        ...
        if not deactivate_names or not self._click_module_menu_button(serial_prefix, *deactivate_names):
            return False
    ...
    def _test_shake_start_button(self) -> Locator:
        ...
        slideout = latch_row.locator("xpath=ancestor::*[.//input[@type='number']][1]")
        for pattern in (r"^Start$", r"^Stop$"):
            btn = slideout.get_by_role("button", name=re.compile(pattern, re.IGNORECASE))
            if btn.count() > 0:
                return btn.first

        return self.page.get_by_role("button", name=re.compile(r"^(Start|Stop)$", re.IGNORECASE)).first
    ...
    def _exercise_heater_shaker_test_shake(self, prefix: str) -> None:
        ...
        try:
            ...
            try:
                expect(start_btn).to_have_text(re.compile(r"stop", re.IGNORECASE), timeout=30_000)
                start_btn.click()
                print("  test shake at 2000 rpm (started and stopped)")
            except AssertionError:
                print("  test shake started but stop control did not appear — deactivating shaker")
    ...
    def _stop_heater_shaker_test_shake(self, prefix: str) -> None:
        ...
        if stop_btn.count() > 0 and stop_btn.is_visible() and re.search(r"stop", stop_btn.inner_text(), re.IGNORECASE):
            stop_btn.click()
    ...
    def _read_module_about_serial(self, module_model: str) -> tuple[str, str | None]:
        ...
        serial_locator = self.page.get_by_test_id(re.compile(rf"^alert_item_serial_{re.escape(module_model)}$")).first
        expect(serial_locator).to_be_visible()
        serial = serial_locator.inner_text().strip()

        version_locator = self.page.get_by_test_id(re.compile(rf"^alert_item_version_{re.escape(module_model)}$"))
        firmware = None
    ...
    def exercise_pipette_card(self, mount: str = "left") -> None:
        ...
        if version_heading.count() > 0:
            firmware = version_heading.first.locator("xpath=following-sibling::p[1]").inner_text().strip()
