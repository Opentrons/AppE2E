...
class DevicesPage(AppBasePage):
    ...
    def robot_card(self) -> Locator:
        ...
        def _by_overflow_menu() -> Locator:
            return self.page.get_by_test_id(f"RobotCard_{self.robot_name}_overflowMenu").locator(
        ...
        return first_resolved((_by_image_id, _by_overflow_menu, _by_robot_name, _by_detail_link))
    ...
    def open_overflow_menu(self) -> None:
        ...
        dismiss_blocking_ui(self.page)
        if not self.robot_detail_url.search(self.page.url):
            self.navigate()
        btn = self.overflow_button()
    ...
    def home_gantry(self) -> None:
        ...
        item = self.page.get_by_test_id(f"RobotOverviewOverflowMenu_homeGantry_{self.robot_name}")
    ...
    def open_robot_settings(self) -> None:
        ...
        self.open_overflow_menu()
        item = self.page.get_by_test_id(f"RobotOverviewOverflowMenu_robotSettings_{self.robot_name}")
        expect(item).to_be_visible()
        item.click()
