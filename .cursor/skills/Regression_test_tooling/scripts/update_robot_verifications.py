...
def get_robot_health(client: httpx.Client, base_url: str, request_timeout: Optional[float] = None) -> dict:
    """Fetch the full /health payload. Short retries on 502/503 (initial connect / quick checks)."""
    data = _get_json_or_none(client, f"{base_url}/health", request_timeout, "/health", verbose=False)
    if data is None:
        raise RuntimeError("GET /health did not return 200 after retries (robot may still be starting)")
    return data
...
def check_update_api_available(client: httpx.Client, base_url: str) -> tuple[bool, Optional[str]]:
...
def _state_snapshot_looks_good(
    ...
    return _instruments_payload_looks_good(instruments_json) and _modules_payload_looks_good(modules_json)
...
def wait_for_stable_robot_state_network(
    ...
    while time.time() - start < overall_timeout:
        ...
        if not _flex_instruments_ready_for_validation(inst_json):
            prev_fingerprint = None
            match_streak = 0
            if not pipette_slow_warn_printed and pipette_slow_warn_after_s > 0 and elapsed >= pipette_slow_warn_after_s:
                print(
...
def wait_for_robot_ready_usb(timeout: float = DEFAULT_WAIT_AFTER_RESTART_S, interval: float = 10) -> None:
