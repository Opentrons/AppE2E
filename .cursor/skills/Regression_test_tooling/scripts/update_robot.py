...
def run_one_update_usb(
    ...
    try:
        status, raw = _serial_request_over_ser(ser, "POST", "/server/update/begin", timeout=begin_timeout)
        if status == 409:
            print("Existing session detected (409). Clearing and retrying...", file=sys.stderr)
            time.sleep(USB_DELAY_BETWEEN_REQUESTS_S)
            _serial_request_over_ser(ser, "POST", "/server/update/cancel", timeout=begin_timeout)
            time.sleep(USB_DELAY_BETWEEN_REQUESTS_S)
            status, raw = _serial_request_over_ser(ser, "POST", "/server/update/begin", timeout=begin_timeout)
        if status != 201:
            ...
            if status == 0:
                ...
                raise RuntimeError(
                    "begin failed: no valid HTTP response (robot may need more time). Raw: {}".format(raw_preview)
                )
    ...
    data = json.loads(raw.decode("utf-8"))
    print(
        f"✅ File received by robot — stage: {STAGE_LABELS.get(data.get('stage'), data.get('stage'))}", file=sys.stderr
    )
    ...
    status, raw = _serial_request_over_ser(ser, "POST", f"/server/update/{token}/commit", timeout=timeout)
    if status != 200:
    ...
    try:
        with httpx.Client(
            ...
            if len(versions) == 1 and current_ver == versions[0]:
                print(
                    f"\n⚠️  Robot is already on version {current_ver}. Proceeding will re-flash the same version.",
                    file=sys.stderr,
                )
                ...
                if local_files is not None and idx < len(local_files):
                    ...
                    try:
                        download_system_file(
                            version,
                            system_file,
                            lambda p: print(f"Download progress: {p:.1f}%", file=sys.stderr),
                        )
                    except Exception:
