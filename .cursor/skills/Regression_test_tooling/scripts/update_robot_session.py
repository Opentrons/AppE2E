...
def download_system_file(version: str, output_path: Path, progress_callback=None) -> None:
...
def wait_for_awaiting_file(client: httpx.Client, base_url: str, token: str, timeout: float = 300) -> None:
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if bad_token_response(exc.response):
                ...
                raise RuntimeError("Update session ended before the robot was ready for file upload.") from exc
            raise
...
def wait_for_done(client: httpx.Client, base_url: str, token: str, timeout: float = 600) -> None:
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if bad_token_response(exc.response):
                ...
                raise RuntimeError("Update session ended unexpectedly on the robot (details above).") from exc
